...
export default class AjaxAutosuggest extends Component {
  ...
  _request = (text ,skipTimer = false) => {
    ...
    if (!skipTimer) this.lastSearchKeyUp = new Date();
    var ms = new Date() - this.lastSearchKeyUp;
    if (!this.lastSearchKeyUp || ms < this.props.debounce) {
        if (this.searchTimer) clearTimeout(this.searchTimer);
        this.searchTimer = setTimeout(function () { 
          selfContext._request.bind(selfContext)(text,true); 
        }, 100);
        return;
    }
    this.lastSearchKeyUp = false;
    this._abortRequests();
  ...
  _renderRowData = (rowData) => {
    ...
    return (
      <Text style={[{flex: 1}, this.props.suppressDefaultStyles ? {} : defaultStyles.suggestion, this.props.styles.suggestion]}
  ...
  render() {
    ...
    var h=Math.min((this._results.length+1)*rowheight,WINDOW.height-this._keyboardHeight-10)
    return (
      <TouchableWithoutFeedback 
